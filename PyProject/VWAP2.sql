...
SELECT vwapString, count(*)
FROM
(SELECT dtdate,strtick,decadjclose,decopen,prevopen,
		--cast(LAG(vwap,5) OVER (partition by strTick order by dtdate ASC) as varchar) +
		cast(LAG(vwap,4) OVER (partition by strTick order by dtdate ASC) as varchar) +
		cast(LAG(vwap,3) OVER (partition by strTick order by dtdate ASC) as varchar) +
		cast(LAG(vwap,2) OVER (partition by strTick order by dtdate ASC) as varchar) +
		cast(LAG(vwap,1) OVER (partition by strTick order by dtdate ASC) as varchar) +
		cast(vwap as varchar)
		AS vwapString
FROM(
SELECT dtdate,strtick,decadjclose,decopen,prevopen,vwap5,CASE WHEN vwap5 < decadjclose THEN 1 ELSE 0 END as VWAP
FROM
(SELECT	dtdate,strTick,decAdjClose,decOpen,LAG(decOpen,1)  OVER (partition by strTick order by dtdate ASC) PrevOpen,
		((decAdjClose * intVol) +
		(LAG(decAdjClose,1) OVER (partition by strTick order by dtdate ASC) *
		LAG(intVol,1) OVER (partition by strTick order by dtdate ASC)) +
		(LAG(decAdjClose,2) OVER (partition by strTick order by dtdate ASC) *
		LAG(intVol,2) OVER (partition by strTick order by dtdate ASC)) +
		(LAG(decAdjClose,3) OVER (partition by strTick order by dtdate ASC) *
		LAG(intVol,3) OVER (partition by strTick order by dtdate ASC)) +
		(LAG(decAdjClose,4) OVER (partition by strTick order by dtdate ASC) *
		LAG(intVol,4) OVER (partition by strTick order by dtdate ASC))) / 
		(intVol+
		LAG(intVol,1) OVER (partition by strTick order by dtdate ASC)+
		LAG(intVol,2) OVER (partition by strTick order by dtdate ASC)+
		LAG(intVol,3) OVER (partition by strTick order by dtdate ASC)+
		...
		as VWAP4,
		intVol/*,
		cast(intVol as decimal)/cast(LAG(intVol,1) OVER (partition by strTick order by dtdate ASC) as decimal)  VolChg*/
FROM VWAP_STAGE
--WHERE strTick in ('TSLA') --and dtdate > dateadd(day,-63,getdate())
WHERE dtdate > '2000-01-01'
		and intVol is not null-- and strtick not in ('HWM','OTIS','CARR','VIAC','PTON','TT','CTVA','ZM','DOW','FOX','FOXA','NLOK','IR','LW','FTV','HPE','PYPL','KHC','UA','WRK','QRVO')
		and intVol <> 0
) vw)vwa
WHERE vwap is not null)ct
GROUP BY vwapString


select strtick,min(dtdate)
from snp500_test
group by strtick
order by min(dtdate) desc
...
delete from Ichimoku3
INSERT INTO Ichimoku3 (strTick,dtBuyDate,decBuyCost,BUY1,BUY2,BUY3,BUY4,BUY5,BUY6,SELL1,SELL2,SELL3,SELL4,SELL5,SELL6)
SELECT strTick,BuyDate,BuyPrice,BUY1,BUY2,BUY3,BUY4,BUY5,BUY6,SELL1,SELL2,SELL3,SELL4,SELL5,SELL6
FROM
(select StrTick,
		...
		CASE	WHEN	decAdjClose > (AVG(decAdjClose) OVER (ORDER BY strtick asc, dtDate ASC ROWS 4 PRECEDING))
				...
				ELSE	0
		END AS BUY5,
		CASE	WHEN	(AVG(decAdjClose) OVER (ORDER BY strtick asc, dtDate ASC ROWS 4 PRECEDING)) is NULL
				THEN	0
				WHEN	decAdjClose > (AVG(decAdjClose) OVER (ORDER BY strtick asc, dtDate ASC ROWS 4 PRECEDING))
				THEN	1
				ELSE	0
		END AS BUY6,
		0 AS SELL1,
...
JOIN (
	...
	SELECT	ID,
			...
			CASE
				WHEN	(	LAG(BUY1,1) OVER (partition by strTick ORDER BY dtBuyDate asc)=1
						AND	LAG(BUY2,1) OVER (partition by strTick ORDER BY dtBuyDate asc)=1
						AND	LAG(BUY3,1) OVER (partition by strTick ORDER BY dtBuyDate asc)=1
						AND	LAG(BUY4,1) OVER (partition by strTick ORDER BY dtBuyDate asc)=1
						AND	LAG(BUY5,1) OVER (partition by strTick ORDER BY dtBuyDate asc)=1
						)
						AND
						(	(BUY2+BUY3+BUY4)<3
						)
				THEN 'SELL'
				WHEN	(	LAG(BUY1,1) OVER (partition by strTick ORDER BY dtBuyDate asc) = 0
						OR	LAG(BUY2,1) OVER (partition by strTick ORDER BY dtBuyDate asc) = 0
						OR	LAG(BUY3,1) OVER (partition by strTick ORDER BY dtBuyDate asc) = 0
						OR	LAG(BUY4,1) OVER (partition by strTick ORDER BY dtBuyDate asc) = 0
						OR	LAG(BUY5,1) OVER (partition by strTick ORDER BY dtBuyDate asc) = 0
						)
						AND
						(	BUY1 = 1
						AND	BUY2 = 1
