...
def getData(ticker):
	#print(ticker)
	try:
		data = pdr.get_data_yahoo(ticker, start=today-timedelta(120), end=today) #18000 in range 30 normal
		#===============================================
		#commented below for testing
		...
		for row in data.itertuples():
			...
			try:
				crsr.execute("""INSERT INTO snp500 (dtDate,decOpen,decHigh,decLow,decClose,decAdjClose,intVol,strTick,ID)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
				tuple(values))
				conn.commit()
