...
SELECT	dtdate,strTick,decAdjClose,decOpen,CL,BL,SPAN_A,SPAN_B,
		LAG(SPAN_A,26) OVER (partition by strTick order by dtdate ASC) SPAN_A_26,
		LAG(SPAN_B,26) OVER (partition by strTick order by dtdate ASC) SPAN_B_26,
		LAG(SPAN_A,52) OVER (partition by strTick order by dtdate ASC) SPAN_A_52,
		LAG(SPAN_B,52) OVER (partition by strTick order by dtdate ASC) SPAN_B_52,
		LAG(decAdjClose,1) OVER (partition by strTick order by dtdate ASC) adjClosePrev,
		LAG(CL,1) OVER (partition by strTick order by dtdate ASC) CLPrev,
		((decAdjClose * intVol) +
		...
		LAG(intVol,4) OVER (partition by strTick order by dtdate ASC))
		as VWAP5,
		LEAD(decAdjClose,9) OVER (partition by strTick order by dtdate ASC)/decOpen as PNL10
FROM ICHIMOKU_STAGE
WHERE strTick in ('AMZN') --and dtdate > dateadd(day,-63,getdate())
ORDER BY dtDate ASC
/*
